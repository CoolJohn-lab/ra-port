...
assert_file_contains PORT/MAC/include/windows.h "#ifndef _lrotl"
...
assert_file_contains PORT/MAC/src/mac_sdl_runtime.cpp "if (!MacRenderer) MacRenderer = SDL_CreateRenderer(MacWindow, -1, SDL_RENDERER_SOFTWARE);"
...
"${cxx_cmd[@]}" "${inc_port[@]}" \
	"$ROOT_DIR/tests/native_view_test.cpp" -o "$tmpdir/native_view_test"

"${cxx_cmd[@]}" -DTRUE_FALSE_DEFINED "${inc_code[@]}" "${inc_port[@]}" \
	"$ROOT_DIR/tests/save_compression_test.cpp" "$ROOT_DIR/CODE/LZOPIPE.CPP" "$ROOT_DIR/CODE/PIPE.CPP" \
	"$ROOT_DIR/CODE/LZO1X_C.CPP" "$ROOT_DIR/CODE/LZO1X_D.CPP" -o "$tmpdir/save_compression_test"
"$tmpdir/save_compression_test"
"${cxx_cmd[@]}" "$ROOT_DIR/tests/lcw_run_test.cpp" "$ROOT_DIR/CODE/LCWUNCMP.CPP" -o "$tmpdir/lcw_run_test"
"$tmpdir/lcw_run_test"
"$tmpdir/native_view_test"

"${cxx_cmd[@]}" "${inc_port[@]}" "$ROOT_DIR/tests/native_input_test.cpp" "$ROOT_DIR/PORT/MAC/src/mac_sdl_runtime.cpp" "$ROOT_DIR/PORT/MAC/src/mac_timer.cpp" $(pkg-config --cflags --libs sdl2) -o "$tmpdir/native_input_test"
"$tmpdir/native_input_test"
